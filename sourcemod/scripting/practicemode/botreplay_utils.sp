...
public void CancelAllReplays() {
  for (int i = 0; i < MAX_REPLAY_CLIENTS; i++) {
    ...
    if (IsValidClient(bot) && BotMimicFix_IsPlayerMimicing(bot)) {
      BotMimicFix_StopPlayerMimic(bot);
...
public void GotoReplayStart(int client, const char[] id, int role) {
  ...
  BMError error = BotMimicFix_GetFileHeaders(filepath, FileHeader, sizeof(FileHeader));
  if (error != BM_NoError) {
    char errorString[128];
    BotMimicFix_GetErrorString(error, errorString, sizeof(errorString));
  ...
  float angles[3];
  float velocity[3];
  Array_Copy(FileHeader.BMFH_initialPosition, origin, 3);
  Array_Copy(FileHeader.BMFH_initialAngles, angles, 3);
