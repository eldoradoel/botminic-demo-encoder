  ...
  if (action == MenuAction_Select) {
    ...
    if (StrEqual(buffer, "replay")) {
      ...
      for (int i = 1; i <= MaxClients; i++) {
        if (IsValidClient(i) && BotMimicFix_IsPlayerMimicing(i)) {
      ...
      int count = 0;
      for (int i = 1; i <= MaxClients; i++) {
        if (IsPlayer(i) && !BotMimicFix_IsPlayerRecording(i)) {
          count++;
        }
      }
      if (count == 0) {
      ...
      int role = 0;
      for (int i = 1; i <= MaxClients; i++) {
        if (IsPlayer(i) && !BotMimicFix_IsPlayerRecording(i) && GetClientTeam(i)) {
...
void FinishRecording(int client, bool printOnFail) {
  if (g_RecordingFullReplay) {
    for (int i = 0; i <= MaxClients; i++) {
      if (IsPlayer(i) && BotMimicFix_IsPlayerRecording(i)) {
        BotMimicFix_StopRecording(i, true /* save */);
      }
    }

  } else {
    if (BotMimicFix_IsPlayerRecording(client)) {
      BotMimicFix_StopRecording(client, true /* save */);
    } else if (printOnFail) {
...
public Action Command_Cancel(int client, int args) {
  ...
  for (int i = 0; i < MAX_REPLAY_CLIENTS; i++) {
    ...
    if (IsValidClient(bot) && BotMimicFix_IsPlayerMimicing(bot)) {
      numReplaying++;
    }
  }
  ...
  } else if (BotMimicFix_IsPlayerRecording(client)) {
    BotMimicFix_StopRecording(client, false /* save */);
...
stock void StartRecording(int client, int role, bool printCommands = true) {
  ...
  BotMimicFix_StartRecording(client, recordName, "practicemode", roleString);
...
public Action BotMimicFix_OnStopRecording(int client, char[] name, char[] category, char[] subdir,
                                char[] path, bool& save) {
  if (g_ReplayPlayRoundTimer[client]) {
  ...
  if (g_CurrentEditingRole[client] >= 0) {
    if (!save) {
      // We only handle the not-saving case here because BotMimicFix_OnRecordSaved below
...
public void BotMimicFix_OnRecordSaved(int client, char[] name, char[] category, char[] subdir, char[] file) {
  if (g_CurrentEditingRole[client] >= 0) {
